...
export class AppMenuComponent implements OnInit {
    ...
    constructor(@Inject(forwardRef(() => AppComponent)) public app:AppComponent, private httpClientService: HttpClientService, private authenticationService: AuthenticationService, private router: Router, private loggingService: LoggingService) {
        this._httpClientSubscription = this.httpClientService.httpClient$.subscribe(
            status => {
                ...
                if (status) {
                    ...
                    for (let group_role of this.group_roles) {
                        if(group_role.id == 1) {
                            for(let role of this.roles.filter(o => o['group_role_id'] == group_role.id)) {
                                this.model.push({label: role.name, icon: role.icon_name, routerLink: [role.router_link]});
                            }
                            continue;
                        }
                        let item = {
                            label: group_role.name, icon: group_role.icon_name,
                            items: []
                        };
                        for(let role of this.roles.filter(o => o['group_role_id'] == group_role.id)) {
                            item.items.push({label: role.name, icon: role.icon_name, routerLink: [role.router_link]});
                        }
